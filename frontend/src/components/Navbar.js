  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 right-4 z-[100] p-2 rounded-lg bg-white/10 backdrop-blur-sm border border-white/20 hover:bg-white/20 transition-all duration-300"
      >
        {isOpen ? (
      ...
      {/* Sidebar */}
      <nav
        className={`fixed top-0 left-0 h-full w-20 lg:w-24 bg-white/5 backdrop-blur-lg border-r border-white/10 transform transition-transform duration-300 ease-in-out lg:translate-x-0 z-[95] ${
        ...
        <div className="flex flex-col items-center py-8 h-full">
          <div className="mb-12">
            <Link to="/" className="logo flex items-center justify-center group" onClick={() => setIsOpen(false)}>
              <FaReact className="w-8 h-8 text-[#61DAFB] transform group-hover:rotate-12 transition-all duration-300" />
            </Link>
          </div>
          ...
          <div className="flex flex-col space-y-8 w-full px-2">
            {navItems.map((item, index) => (
              <Link
                ...
                onClick={() => setIsOpen(false)}
              >
                <div className={`p-3 rounded-xl ${item.bgColor} ${item.hoverBgColor} transition-all duration-300`}>
                  <div className={`${item.color} ${item.hoverColor} transform group-hover:scale-110 transition-all duration-300`}>
                    {item.icon}
                  </div>
                </div>
                <span className="absolute left-full ml-2 px-2 py-1 bg-white/10 backdrop-blur-sm rounded-lg text-sm font-medium text-white opacity-0 group-hover:opacity-100 transition-all duration-300 whitespace-nowrap">
                  {item.label}
                </span>
              </Link>
            ))}
          ...
          <div className="mt-auto flex flex-col space-y-4 w-full px-2">
            {socialLinks.map((item, index) => (
              <a
                ...
                className="group relative flex items-center justify-center"
              >
                <div className={`p-2 rounded-xl ${item.bgColor} ${item.hoverBgColor} transition-all duration-300`}>
                  <div className={`${item.color} ${item.hoverColor} transform group-hover:scale-110 transition-all duration-300`}>
                    {item.icon}
                  </div>
                </div>
